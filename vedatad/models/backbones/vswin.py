...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("dummy_tensor", dummy_tensor)

    def forward_part1(self, x, mask_matrix, dummy_tensor):
        assert dummy_tensor is not None
        B, D, H, W, C = x.shape
    ...
    def forward_part2(self, x, dummy_tensor):
        assert dummy_tensor is not None
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            x = checkpoint.checkpoint(
                self.forward_part1, x, mask_matrix, self.dummy_tensor
            )
        else:
        ...
        if self.use_checkpoint:
            x = x + checkpoint.checkpoint(self.forward_part2, x, self.dummy_tensor)
        else:
            x = x + self.forward_part2(x, self.dummy_tensor)
...
class SwinTransformer3D(nn.Module):
    ...
    Args:
        ...
        out_indices (Sequence[int]): indices of output feature. Indices are in range [2,4].
            Default to (4,). 4 means stage4. If only one indices, one tensor is output, else a tuple
            of tensors is outputted.
    """
    ...
    def __init__(
        ...
        out_num_channels: Sequence[int] = (384, 786, 786),
    ):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                use_checkpoint=use_checkpoint if i_layer >= frozen_stages else None,
            )
