...
# log_file = osp.join(checkpoint_path, f"{timestamp}.log")
...
def calc_bce_loss(start, end, scores):
    ...
    # start = torch.sigmoid(start.mean(-1))
    # end = torch.sigmoid(end.mean(-1))

    # bound to [0,1]
    # start = F.sigmoid(start)
    # end = F.sigmoid(end)
    # print("start", start, scores[:, 1])
    # print("end", end, scores[:, 2])

    # assert scores[:,1].max() <= 1, "scores must between [0,1]"
    # assert scores[:,2].max() <= 1, "scores must between [0,1]"
...
def forward_one_epoch(
    net, clips, targets, CPD_Loss, action_class_loss, scores=None, training=True, ssl=True
):
    clips = clips.cuda()
...
def run_one_epoch(
    ...
    for n_iter, (
        ...
        (
            ...
            net, clips, targets, CPD_Loss, action_class_loss, scores, training=training, ssl=False
        )
        ...
        if rank == 0:
            # pbar.set_postfix(
            #     loss="{:.5f}".format(float(cost.cpu().detach().numpy())),
            #     loss_l="{:.5f}".format(float(loss_l.cpu().detach().numpy())),
            #     loss_c="{:.5f}".format(float(loss_c.cpu().detach().numpy())),
            #     loss_prop_l="{:.5f}".format(float(loss_prop_l.cpu().detach().numpy())),
            #     loss_prop_c="{:.5f}".format(float(loss_prop_c.cpu().detach().numpy())),
            #     loss_ct="{:.5f}".format(float(loss_ct.cpu().detach().numpy())),
            #     loss_start="{:.5f}".format(float(loss_start.cpu().detach().numpy())),
            #     loss_end="{:.5f}".format(float(loss_end.cpu().detach().numpy())),
            #     loss_action="{:.5f}".format(float(loss_action.cpu().detach().numpy())),
            # )
            pbar.set_postfix(
    ...
    # loss_loc_val /= epoch_step_num + 1
    # loss_conf_val /= epoch_step_num + 1
    # loss_prop_l_val /= epoch_step_num + 1
    # loss_prop_c_val /= epoch_step_num + 1
    # loss_ct_val /= epoch_step_num + 1
    # loss_start_val /= epoch_step_num + 1
    # loss_end_val /= epoch_step_num + 1
    # loss_trip_val /= epoch_step_num + 1
    # cost_val /= epoch_step_num + 1
    # loss_action_val /= epoch_step_num + 1
...
def main(local_rank):
    ...
    Setup model
    """
    net = BDNet(model_cfg=config["other_config"]).to(device)
